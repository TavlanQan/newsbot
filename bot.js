...
async function handleYouTubeRemove(ctx, input) {
  try {
    ...
    if (youtubeFeeds.length === 0) {
      ctx.reply('❌ Нет YouTube-каналов для удаления.', youtubeMenu);
      return;
    }
...
bot.hears('📋 Список YouTube', async (ctx) => {
  try {
    const youtubeFeeds = await getYouTubeFeeds();
    if (youtubeFeeds.length === 0) {
      ctx.reply('📺 Нет добавленных YouTube-каналов.', youtubeMenu);
      return;
    }
    ...
    ctx.reply('❌ Ошибка при получении списка YouTube-каналов.', youtubeMenu);
  }
});
...
bot.hears('🔄 Запустить пересылку', async (ctx) => {
  try {
    ...
    ctx.reply('❌ Ошибка при активации пересылки.', mainMenu);
  }
});
...
bot.hears('⏹️ Остановить пересылку', async (ctx) => {
  try {
    ...
    ctx.reply('❌ Ошибка при остановке пересылки.', mainMenu);
  }
});
...
bot.hears('🗝️ Ключевые слова', async (ctx) => {
  try {
    ...
    ctx.reply('❌ Ошибка при получении ключевых слов.', mainMenu);
  }
});
...
bot.hears('🎯 Целевые каналы', async (ctx) => {
  try {
    ...
    ctx.reply('❌ Ошибка при получении целевых каналов.', mainMenu);
  }
});
...
bot.hears('📡 Мониторинг каналов', async (ctx) => {
  try {
    ...
    ctx.reply('❌ Ошибка при получении отслеживаемых каналов.', mainMenu);
  }
});
...
bot.on('message', async (ctx) => {
  ...
  try {
    ...
    if (text.toLowerCase() === 'отмена' && state) {
      userStates.delete(userId);
      ctx.reply('❌ Действие отменено. Возвращаюсь в меню.', state === 'waiting_for_youtube_link' || state === 'waiting_for_youtube_remove' ? youtubeMenu : mainMenu);
      return;
    }
